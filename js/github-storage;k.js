...
const GHS = (() => {
  ...
  /* ══ 保存ラッパー ══
   * localStorage 保存は即時成功。GitHub 保存は PAT があれば試みる。
   * GitHub 失敗でも localStorage 保存済みなので onSaved を呼ぶ。
   */
  async function saveWithPAT(dateKey, data, onSaved, onError) {
    // まず localStorage に保存（常に成功）
    lsSave(dateKey, data);
    if (onSaved) onSaved('local'); // ← 即座にコールバック

    // GitHub への同期（PAT がない場合はスキップ）
    if (!hasPAT()) return;

    const repo = detectRepo();
    if (!repo) return; // ローカル環境はスキップ
      ...
      if (e.message === 'PAT_REQUIRED' || e.message === 'PAT_INVALID') {
        ...
        showPATModal(async () => {
          try {
            ...
            // GitHub 失敗でも localStorage には保存済みなので無視
            console.warn('GitHub sync failed:', e2.message);
          }
        });
      }
      // その他エラーは localStorage 保存済みなので無視
    }
  }
